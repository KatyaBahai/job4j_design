...
class ArgsNameTest {
    ...
    void whenNoKeyThenExceptionThrown() {
        assertThatThrownBy(() -> ArgsName.of(new String[] {"-=ioioio", "-hj=hj"}))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageMatching("^.+")
                .hasMessageContaining("Error: This argument '-=ioioio' does not contain a key");
    }

    @Test
    void whenNoValueThenExceptionThrown() {
        assertThatThrownBy(() -> ArgsName.of(new String[]{"-dfhjg=", "-encoding="}))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageMatching("^.+")
                .hasMessageContaining("Error: This argument '-dfhjg=' does not contain a value");
    }

    @Test
    void whenNoHyphenBeforeKeyThenExceptionThrown() {
        assertThatThrownBy(() -> ArgsName.of(new String[]{"-Xmx=512", "fkfl==="}))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageMatching("^.+")
                .hasMessageContaining("Error: This argument 'fkfl===' does not start with a '-' character");
    }

    @Test
    void whenNoEqualsThenExceptionThrown() {
        assertThatThrownBy(() -> ArgsName.of(new String[]{"-sds=90765", "-request?msgHello"}))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageMatching("^.+")
                .hasMessageContaining("Error: This argument '-request?msgHello' does not contain an equal sign");
    }
